...
for k in range(len(dghpmap)):
    ...
    if(b==0):
        ...
        if(Dir_Ex[k] > 1200):
            ...
            """
            if(k == 2100):
                print("2100 : %d, %lf,  %lf, %lf, %lf"%(n,b,UL,FUL,Dir_Ex[k]))
            elif(k == 2150):
                print("2150 : %d, %lf, %lf, %lf, %lf"%(n,b,UL,FUL,Dir_Ex[k]))
            elif(k == 2200):
                print("2200 : %d, %lf, %lf, %lf, %lf"%(n,b,UL,FUL,Dir_Ex[k]))
            """
        else:
            FUL = 100
...
plt.title(r"Log-Scale Flux Upper Limit projection : 972 Bins, E > 3EeV",fontsize=30)
