...
"""
D_E = [5800,5550,4550,4250,4150,
       4100,4100,4100,4100,4100,
       4100,4100,4000,3900,3800,
       3750,3650,3600,3550,3500,
       3450,3350,3250,3150,3050,
       2950,2800,2650,2500,2300,
       2100,1900,1750,1500,1250,
       1050,800,500,50,50,50,50,50,50,50,50,50,50,50,50,50,50,50,50,50,50,50,50,50,50] # Directional Exposure
"""
...
df3E1=pd.read_csv('Cut2to3EeV.csv')
E3L=df3E1['L'].values
E3B=df3E1['B'].values

df3E2=pd.read_csv('test2to3EeV.csv')
E3bL=df3E2['L'].values
E3bB=df3E2['B'].values
E3bdL=df3E2['dL'].values
E3bdB=df3E2['dB'].values

df3E3=pd.read_csv('JF12_2to3EeVCut.csv')
...
Um21 = np.nanmean(FUL3_l[0:24]) * 1000
U15 = np.nanmean(FUL3_l) * 1000
...
plt.title('Mean Flux Upper Limit, 2EeV < E $\leq$ 3EeV, nside = 6', fontsize=30)
plt.step(Dec3_l, FUL3_l, where='mid', color='red', label=label, linewidth=1.5)

print(np.nanmean(FUL3_l[0:24]), np.nanmean(FUL3_l))
