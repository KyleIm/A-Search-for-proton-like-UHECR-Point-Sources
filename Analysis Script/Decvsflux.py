...
bdL=df2['dL'].values # Deflected Background L
bdB=df2['dB'].values
...
for k in range(len(dghpmap)):
    if(dghpmapb[k]>0):
        ...
        #print(n,b,UL)
        if(b != 0 and Dir_Ex[k] >= 1250 and UL > 0.000000001):
            FUL = UL / Dir_Ex[k]
...
Dec_l, FUL_l = compute_bin_averages(dgULmap_l)

Um21 = np.mean(Um21_l) * 1000
U15 = np.mean(U15_l) * 1000
...
plt.title(r'Mean Flux Upper Limit, E > 5 EeV, $N_{side}$ = 15', fontsize=30)
plt.step(Dec_l, FUL_l, where='mid', color='red', label=label, linewidth=1.5)
