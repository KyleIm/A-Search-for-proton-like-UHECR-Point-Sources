...
namespace crpropa {
  ...
  double randomfield(double mean, double std) {
...
JF12Field::JF12Field(bool rand_field) {
  useRegularField = true;
	...
	double bRingmean = 0.1;
	double bRingstd = 0.1;
	double hDiskmean = 0.40;
	double hDiskstd = 0.03;
	double wDiskmean = 0.27;
	double wDiskstd = 0.08;
	...
	if (rand_field == true)
	{  
	  bRing = randomfield(bRingmean,bRingstd) * muG;
	  hDisk = randomfield(hDiskmean,hDiskstd) * kpc;
	  wDisk = randomfield(wDiskmean,wDiskstd) * kpc;
	  bDisk[0] = randomfield(b1mean,b1std) * muG;
	  bDisk[1] = randomfield(b2mean,b2std) * muG;
	  bDisk[2] = randomfield(b3mean,b3std) * muG;
	  bDisk[3] = randomfield(b4mean,b4std) * muG;
	  bDisk[4] = randomfield(b5mean,b5std) * muG;
	  bDisk[5] = randomfield(b6mean,b6std) * muG;
	  bDisk[6] = randomfield(b7mean,b7std) * muG;
	  bDisk[7] = randomfield(b8mean,b8std) * muG;
	  bNorth = randomfield(bNmean,bNstd) * muG;
	  bSouth = randomfield(bSmean,bSstd) * muG;
	  rNorth = randomfield(rNmean,rNstd) * kpc;
	  rSouth = 17 * kpc; //Only written to be > 16.7
	  wHalo = randomfield(wHmean,wHstd) * kpc;
	  z0 = randomfield(z0mean,z0std) * kpc;
	  bX = randomfield(bXmean, bXstd) * muG;
	  thetaX0 = randomfield(thetaX0mean, thetaX0std) * M_PI / 180;
	  sinThetaX0 = sin(thetaX0);
	  cosThetaX0 = cos(thetaX0);
	  tanThetaX0 = tan(thetaX0);
	  cotThetaX0 = 1. / tanThetaX0;
	  rXc = randomfield(rXcmean,rXcstd) * kpc;
	  rX = randomfield(rXmean,rXstd) * kpc;
